...
nock(nconf.get('AUTH_URI'), {
  ...
  'profile'          : {
    'name'        : 'admin',
    'slug'        : 'admin',
    'permissions' : ['changeCourse', 'changeDiscipline', 'changeCatalog', 'changeModality']
  }
});
    ...
    it('should list', function (done) {
      ...
      request.expect(function (response) {
        ...
        response.body.every(function (modality) {
          ...
          modality.should.have.property('catalog').with.property('year');
        });
      });
      request.end(done);
    });
  ...
  describe('details', function () {
    ...
    it('should show', function (done) {
      ...
      request.expect(function (response) {
        ...
        response.body.should.have.property('catalog').with.property('year').be.equal(2014);
      });
      request.end(done);
    });
  });
